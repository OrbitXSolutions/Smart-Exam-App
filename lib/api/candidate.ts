...
export async function getAvailableExams(): Promise<CandidateExam[]> {
  ...
  const response = await apiClient.get<CandidateExam[]>("/Candidate/exams")
  console.log("[v0] Available exams loaded:", response?.length || 0)
  return response
}
...
export async function getMyResults(): Promise<CandidateResultDto[]> {
  ...
  const response = await apiClient.get<CandidateResultDto[]>("/Candidate/results/my-results")
  console.log("[v0] Results loaded:", response?.length || 0)
  return response
}
...
/**
 * Log attempt event (for proctoring)
...
export async function logAttemptEvent(attemptId: number, event: LogAttemptEventRequest): Promise<boolean> {
  console.log("[v0] Log event (local only):", attemptId, event.eventType)
  // Events are logged locally for now - server endpoint may not exist
  return true
}
