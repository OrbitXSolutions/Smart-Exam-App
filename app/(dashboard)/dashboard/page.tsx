...
import { mockDashboardStats, mockExams, mockAttempts, mockIncidentCases } from "@/lib/mock-data"
import {
  ClipboardList,
  Users,
  CheckCircle,
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {mockExams
                .filter((e) => e.isPublished && e.startAt)
                .slice(0, 3)
                .map((exam) => (
                  <div
                    key={exam.id}
              ...
              {mockExams.filter((e) => e.isPublished && e.startAt).length === 0 && (
          ...
          {isAdmin ? (
            <Card>
              ...
              <CardContent className="space-y-4">
                {mockIncidentCases.slice(0, 3).map((incident) => (
                  <div
                    key={incident.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{getLocalizedField(incident, "title", language)}</p>
                  ...
                  >
                    <div
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${
                        attempt.isPassed ? "bg-green-500/10" : attempt.status === 1 ? "bg-yellow-500/10" : "bg-muted"
                      }`}
                    >
                      {attempt.isPassed ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{getLocalizedField(attempt, "examTitle", language)}</p>
                      <p className="text-xs text-muted-foreground mt-1">{attempt.statusName}</p>
                    </div>
                    {attempt.score !== undefined && (
                      <span className={`font-medium ${attempt.isPassed ? "text-green-500" : "text-destructive"}`}>
                        {attempt.score}%
                      </span>
                    )}
                  </div>
